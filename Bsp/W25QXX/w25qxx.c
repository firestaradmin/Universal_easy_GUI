...
{	
	...
	RCC_APB2PeriphClockCmd(	W25QXX_CS_GPIO_Periph, ENABLE );

	GPIO_InitStructure.GPIO_Pin = W25QXX_CS_Pin;  // PA4 推挽 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
 	GPIO_SetBits(W25QXX_CS_GPIO,W25QXX_CS_Pin);
 
    W25QXX_CS_DISABEL;				//SPI FLASH不选中
	SPI1_Init();		   	//初始化SPI
...
{  
	u8 byte=0;   
	W25QXX_CS_ENABEL;                            //使能器件   
	SPI1_ReadWriteByte(W25X_ReadStatusReg); //发送读取状态寄存器命令    
	byte=SPI1_ReadWriteByte(0Xff);          //读取一个字节  
	W25QXX_CS_DISABEL;                            //取消片选     
	return byte;   
} 
//写W25QXX状态寄存器
